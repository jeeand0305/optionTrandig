...
# =================================================
    ...
    def get_active_open_options3(self) -> dict:
        ...
        try:
            ...
            logger.error(f"Критическая ошибка при сканировании options позиций: {e}")
    ...
    def process_hedging_logic3(self, 
                               optionsAll,
                               futuresAll) -> bool:
        """
        ГЛАВНЫЙ ДИСПЕТЧЕР (Версия 2): Проверяет данные, 
        итерирует портфель по монетам и изолирует ошибки через try/except.
        
        ЛОГИКА ВОЗВРАТА:
        True  -> Есть проблема/риск (требуется реакция на следующем шаге).
        ...
        if not self.is_options_data_valid(options_dict=optionsAll): 
            logger.error("🚨 Данные опционов невалидны!")
            return True
            
        if not self.is_futures_data_valid(futures_dict=futuresAll):
            logger.error("🚨 Данные фьючерсов невалидны!")
            return True

        # Если портфель опционов пуст — проблем нет, позиция чистая (False)
        if not optionsAll:
            logger.info("ℹ️ Портфель опционов пуст. Рисков нет (False).")
            return False
        
        has_any_problem = False  # Переменная для отслеживания проблем по всему портфелю

        # 2. Перебираем сгруппированный словарь опционов по монетам
        for coin_name, options_list in optionsAll.items():
            coin_upper = coin_name.upper()
            
            # Пропуск монет со встроенным DDH Bybit
            ...
            try:
                ...
                # === Проверка инвертированного коридора ===
                ...
                # -----------------------------------------------------------------
                # ШАГ 3: ШТАТНЫЙ РЕЖИМ (Если инверсии НЕТ)
                # -----------------------------------------------------------------
                    if coin_data['put_strike'] <= coin_data['call_strike']:
                        coridor_result = self.analizCoridorStrikes2(coin_data)
                        logger.info(f" coridor_result {coridor_result}")
                        
                        # Если аналитика подтвердила проблему в коридоре
                        if coridor_result.get('analizeBool') == True:
                            has_any_problem = True
                        if coridor_result.get('analizeBool') == False:
                            continue
                    ...
                    if coin_data['call_strike'] <= coin_data['ticPrice']:
                        ...
                        if call_result.get('analizeBool') == True:
                            has_any_problem = True
                        if call_result.get('analizeBool') == False:
                            logger.info(f"✅ {coin_upper} Call-хеджирование в норме")
                            continue
                        
                    if coin_data['put_strike'] >= coin_data['ticPrice']:
                        put_result = self.analizPutStrike(coin_data) # Допустим, возвращает dict с аналайзом
                        logger.info(f"✅ {coin_upper} Put-риски проверены")
                        # Если у тебя analizPutStrike тоже возвращает флаг, проверяй его тут
                        # has_any_problem = True
                        continue                    
                
                # Если всего один продольный страйк КОЛ или ПУТ
                elif (coin_data['put_strike'] is not None 
                       or coin_data['call_strike'] is not None): 
                    logger.info(f"otrobotka otdelnih stirkov  ne v condore")
                    if coin_data['call_strike'] is not None:
                        ...
                        if call_result.get('analizeBool') == True:
                            ...
                            # continue
                        if call_result.get('analizeBool') == False:
                            logger.info(f"✅ {coin_upper} Одиночный Call в норме")
                            continue
                        
                    if coin_data['put_strike'] is not None:
                        # and coin_data['put_strike'] >= coin_data['ticPrice']):
                        self.analizPutStrike(coin_data)
                        logger.info(f"✅ {coin_upper} Одиночный Put проверен")
                        continue   
                ...
                # Настоящий критический сбой — это 100% проблема для следующего шага
                logger.error(f"💥 Критический сбой при обработке монеты {coin_upper}: {coin_error}")
                has_any_problem = True
                continue  
            
        # Конец главного цикла. Возвращаем True, если ХОТЯ БЫ по одной монете есть проблема
        return has_any_problem
    ...
    def get_active_open_options3(self) -> dict:
        ...
        """
        # Принудительно импортируем наш отлаженный универсальный класс-парсер
        logger.info("get_active_open_options3(self) -> dict:")
        logger.debug("Запуск проверки открытых, неэкспирированных и живых опционов...")
        active_options = []
        list_active_option = []
        total_active_option = {}
    ...
    def process_hedging_logic2(self, 
                               optionsAll,
                               futuresAll) -> bool:
        """
        ГЛАВНЫЙ ДИСПЕТЧЕР (Версия 2): Проверяет данные, 
    ...
    def analizCallStrike(self, coin_data: dict, midStrike = None):
        ...
        ticPrice        = coin_data['ticPrice']#
        open_futures    = coin_data['open_futures'] #
        futures_symbol  = coin_data['optionsSellCall'][0].get('futures_symbol', 'error/1893')#
        side_call       = 'buy'
        ...
        if ticPrice >= call_strike:
            ...
            logger.error(f"пероблем в отлове ПУТ опциона")
    ...
    def analizPutStrike(self, coin_data: dict, 
        ...
        ticPrice       = coin_data['ticPrice']
        open_futures   = coin_data['open_futures']
        futures_symbol = coin_data['optionsSellPut'][0].get('futures_symbol', 'error/2126')
        side_put       = 'sell'
        ...
        if ticPrice <= put_strike:
            ...
            logger.info(f" SHORT хэдж. не нужен")
            return False
    ...
    def aggregate_coin_data(self, optionsList: list, 
                            nameCoin: str,
                            futuresAll) -> dict:
        ticPrice = self.get_ticker_by_symbol(symbol=nameCoin)
    ...
    #chekaem kak ispolzuetca metod open position futures
    # hedge - false, one way = true   
    def check_position_mode_direct(self, target_symbol:  str = 'XRP/USDT:USDT') -> bool:
        """
        УНИВЕРСАЛЬНЫЙ ЧЕКЕР РЕЖИМА (Прямая лаконичная версия):
        Принимает готовый CCXT-символ фьючерса из твоего класса OptionAsset.
        ...
        try:
            ...
            if positions[0].get('hedged') is True:
                ...
                logger.error(f"⚠️ Непредвиденный формат флага: {positions[0].get('hedged')}. Риск (True).")
                return True

        except Exception as e:
            # Сюда прилетит IndexError (если рынок пуст) и любые сетевые сбои API
            logger.error(f"💥 Сбой API или структуры данных для {target_symbol}: {e}")
            return True  # Сбой — это всегда риск (True)


    def set_position_mode_to_hedge_direct(self, target_symbol: str = 'XRP/USDT:USDT') -> bool:
        """
        УНИВЕРСАЛЬНЫЙ ПЕРЕКЛЮЧАТЕЛЬ (Чистая версия):
        Принимает уже готовый CCXT-символ фьючерса из твоего класса OptionAsset.
        ...
        try:
            ...
            # Отправляем унифицированную команду CCXT на включение Хеджа (hedged=True)
            response = self.exchange.set_position_mode(hedged=True, symbol=target_symbol)
            
            logger.info(f"🎉 Режим успешно изменен на HEDGE MODE для {target_symbol}. Ответ: {response}")
            return False # Проблем нет

        except ccxt.MarginModeAlreadySet:
            # Если Хедж уже включен, биржа/CCXT выбросит этот эксепшн
            logger.info(f"✅ Режим HEDGE MODE для {target_symbol} уже активен. Изменения не требуются.")
            return False # Проблем нет
...
# getD = bybitOpt.place_futures_hedge_order( #work  one way
#     base_currency='XRP',
#     side="sell",
#     qty=10.0,)
